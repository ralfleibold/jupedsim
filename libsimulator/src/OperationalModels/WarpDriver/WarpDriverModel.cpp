    ...
    const AgentStep& step) const
{
    ...
    Point toTarget = step.to_next_target();
    ...
    const auto neighbors = step.other_agents_in_range(_cutOffRadius);
    ...
    for(const auto& neighbor : neighbors) {
        ...
        WarpParams wp{};
        wp.relPosB = neighbor.relative_position;
    ...
    if(detourTime > 0.0) {
        ...
        if(!step.inside_geometry(movement)) {
