...
/// *forward* (y in [10,15], z = y-10). Tilted but its (x,y)-footprint does NOT
/// overlap the floor -> the whole surface is single-valued -> one region.
...
/// floor + ramp. The ramp's footprint is disjoint from the lower floor's, so
...
/// Mean z of a face's vertices -- used to tell floor faces (z=0) from ramp faces.
...
{
    // The key case the user asked about: a whole stair/ramp is single-valued
    // with a floor and must NOT get its own region -- it merges across the
    // (edge-welded) seams. Only the overlapping upper floor splits off.
