...
{
    const auto& variant = static_cast<const OperationalModelStateVariant&>(state);
...
{
    py::object _repack;
    /// Deque, not vector: state_of() hands out references that must survive later calls.
    mutable std::deque<OperationalModelState> _states{};

public:
    explicit PythonNeighborStates(py::object repack) : _repack(std::move(repack)) {}

    const OperationalModelState& state_of(const GenericAgent& agent) const override
    {
        py::object repacked = _repack(stateToPython(agent.model));
        try {
            _states.emplace_back(repacked.cast<OperationalModelStateVariant>());
    ...
    py::class_<AgentStep, AgentView>(m, "AgentStep")
        .def(
            ...
            [](const AgentStep& self, const PythonNeighborStates& states) {
                return self.with_neighbor_states(states);
            },
