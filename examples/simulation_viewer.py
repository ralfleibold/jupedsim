...
class SimulationViewer:
    ...
    def __init__(
        ...
        *,
        ...
        self._plotter.enable_terrain_style(mouse_wheel_zooms=True, shift_pans=True)
        self._server = None  # built lazily in run()
        self._agents_mesh = None  # glyph dataset, updated in place per frame (see _draw_agents)
        self._floor_mesh = None  # static surface mesh, loaded once (see _load_floor_mesh)
        self._n_regions = 0
    ...
    def _load_floor_mesh(self) -> None:
        ...
        geo = Geometry3D()
        geo.initialize_from_obj(str(self._geometry_obj))
        ...
        mesh["elevation"] = verts[:, 2]  # colour by height (point scalar), like trame_viewer
        region_ids = np.asarray(geo.region_ids(), dtype=float)
    ...
    def _agent_points(self) -> np.ndarray:
        pts = [
            (ag.position[0], ag.position[1], ag._obj.z)
    ...
    def _build_server(self):
        ...
        state.steps_per_frame = self._steps_per_frame  # live speed knob (toolbar)
        state.wireframe = True  # show the triangulation edges
        state.color_by_region = False  # False -> colour by height, True -> by region id
        ...
        async def _run_loop() -> None:
            ...
            while state.playing:
                ...
                try:
                    ...
                    n = self._steps_per_frame  # mid-typing in the field: keep the last good value
                for _ in range(n):
                    self._sim.iterate()
    ...
    def run(self, *, port: Optional[int] = None, window_size=(1200, 800)) -> None:
