...
def build_app(obj_path: str | None):
    ...
    # Routing interaction state (start vertex index, whether we are live-tracking the target).
    route: dict = {
        "start_idx": None,
        "target_idx": None,
        "tracking": False,
    ...
    def load_mesh(mesh, name: str) -> None:
    ...
    def _on_file(obj_file, **_):
        ...
        with tempfile.NamedTemporaryFile(suffix=".obj", delete=False) as tmp:
            ...
            tmp_path = tmp.name
        try:
            load_mesh(pv.read(tmp_path), client_file.name)
        finally:
            os.unlink(tmp_path)
    ...
    # -- routing (Tier 1: VTK geodesic over the loaded surface) --------------------------------

    def clear_route() -> None:
        route.update(start_idx=None, target_idx=None, tracking=False)
        ...
        with state:
            ...
            state.route_length, state.route_corners, state.route_us = 0.0, 0, 0
    ...
    def _vertex_from_pick():
        """Most recent pick -> full-mesh vertex, snapping to a corner of the *picked triangle*."""
        ds = (
            picker.GetDataSet()
        )  # may be a clipped sub-mesh, so go via world coords
        ids = ds.GetCell(picker.GetCellId()).GetPointIds()
        corners = np.array(
            [ds.GetPoint(ids.GetId(i)) for i in range(ids.GetNumberOfIds())]
        )
        pos = np.asarray(picker.GetPickPosition())
        corner = corners[int(np.argmin(np.linalg.norm(corners - pos, axis=1)))]
        return int(current["mesh"].find_closest_point(corner))

    def update_route(target_idx: int) -> None:
        mesh = current["mesh"]
        start_idx = route["start_idx"]
        if start_idx is None or target_idx == start_idx:
            return
        t0 = time.perf_counter()
        try:
            path = mesh.geodesic(start_idx, target_idx)
        except Exception:
            return  # no path (e.g. pick on a disconnected piece)
        ...
        plotter.add_mesh(
            path, name="route_path", color="red", line_width=6, pickable=False
        )
        _add_marker("route_target", mesh.points[target_idx], "red")
        with state:
            state.route_target = _fmt(mesh.points[target_idx])
            state.route_length = round(length, 2)
            state.route_corners = int(len(pts))
            state.route_us = int(elapsed_us)
        if ctrl.view_update:
            ctrl.view_update()

    def set_start(start_idx: int) -> None:
        route.update(start_idx=start_idx, tracking=True, target_idx=None)
        ...
        _add_marker(
            "route_start", current["mesh"].points[start_idx], "limegreen"
        )
        with state:
            state.route_status = "tracking"
            state.route_start = _fmt(current["mesh"].points[start_idx])
            state.route_target = "—"
            state.route_length, state.route_corners, state.route_us = 0.0, 0, 0
        if ctrl.view_update:
            ctrl.view_update()
    ...
    def _route_hover(fx, fy):
        ...
        if pos is None or not route["tracking"]:
            return
        idx = _vertex_from_pick()
        if idx is not None and idx != route["target_idx"]:
            update_route(idx)
    ...
    def _route_click(fx, fy):
        ...
        if route["tracking"]:
            ...
            if ctrl.view_update:
                ctrl.view_update()
            return
        if _pick_world_fraction(fx, fy) is not None:
            set_start(_vertex_from_pick())
    ...
    with SinglePageWithDrawerLayout(server) as layout:
        ...
        with layout.drawer:
            with v3.VList(density="compact"):
                ...
                v3.VListItem(
                    "{{ route_length.toFixed(2) }} m", subtitle="Path length"
                )
                v3.VListItem("{{ route_corners }}", subtitle="Corners")
    ...
    if obj_path:
        load_mesh(pv.read(obj_path), os.path.basename(obj_path))
    return server
