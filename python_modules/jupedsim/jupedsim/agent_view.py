...
class AgentView:
    ...
    Example — visibility-filtered neighborhood::
        ...
        def compute_next_state(self, state, step):
            neighbors = step.other_agents_in_range(
                5.0,
                lambda n: step.no_geometry_between(n.relative_position),
            )
    """
    ...
    def no_geometry_between(
        ...
        *relative_position* away is not intersected by any geometry boundary.

        Args:
            relative_position: Offset from the agent as ``(dx, dy)``.
        """
        return self._obj.no_geometry_between(relative_position)
...
class AgentStep(AgentView):
    ...
    def with_neighbor_states(self, repack: Callable[[Any], Any]) -> "AgentStep":
        ...
        """
        return AgentStep(
            self._obj.with_neighbor_states(py_jps._NeighborStates(repack))
        )
