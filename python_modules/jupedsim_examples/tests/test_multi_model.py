...
class MultiModel(jps.CustomOperationalModel):
    ...
    def compute_next_state(self, state, step):
        if state.kind == "social_force":
            sub_step = step.with_neighbor_states(self._as_social_force_state)
            sub, movement = self._social_force.compute_next_state(
                state.sub, sub_step
            )
        else:
            sub_step = step.with_neighbor_states(self._as_collision_free_state)
...
def test_each_agent_keeps_its_own_state_type():
    ...
    assert isinstance(sim.agent(left).model.sub, PythonSocialForceModelState)
    assert isinstance(
        sim.agent(right).model.sub, jps.CollisionFreeSpeedModelV3State
    )
